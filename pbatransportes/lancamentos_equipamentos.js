...
console.log('📦🔥 NOVA VERSÃO 29/01/2026 18:05 - Carregando lancamentos_equipamentos.js com recálculo em tempo real!');
...
const updateAlertBadges = (card) => {
    ...
    const stoppagesCount = stoppagesContainer ? 
        Array.from(stoppagesContainer.querySelectorAll('.form-grid')).filter(row => {
            const typeSelect = row.querySelector('.stoppage-type');
            return typeSelect && typeSelect.value;
        }).length : 0;
    
    if (stoppagesCount > 0) {
        alertBadges.push(`<span class="alert-badge alert-badge-stoppage">⚠️ PARADA REGISTRADA${stoppagesCount > 1 ? ` (${stoppagesCount})` : ''}</span>`);
    }
    
    // Contar observações/despesas/acréscimos
    const notesContainer = card.querySelector('.notes-container');
    let observacoesCount = 0, despesasCount = 0, acrescimosCount = 0;
    
    if (notesContainer) {
        notesContainer.querySelectorAll('.form-grid').forEach(row => {
            const noteTypeSelect = row.querySelector('.note-type');
            if (noteTypeSelect && noteTypeSelect.value) {
                if (noteTypeSelect.value === 'observation') observacoesCount++;
                else if (noteTypeSelect.value === 'expense') despesasCount++;
                else if (noteTypeSelect.value === 'addition') acrescimosCount++;
            }
        });
    }
...
const getEquipmentMobilizationStatus = async (equipmentId, workId, startDate, endDate) => {
    try {
        ...
        // 🔴 DESMOBILIZADO: Se tem desmobilização E não trabalhou depois
        if (hasDemobilization) {
            // Encontrar data da última desmobilização
            ...
            // Verificar se trabalhou DEPOIS da desmobilização OU foi remobilizado
            const entriesAfterDemob = allEntries.filter(e => 
                (e.is_worked || e.is_mobilization) && new Date(e.date) > new Date(lastDemobDate)
            );
            
            if (entriesAfterDemob.length > 0) {
                console.log(`   [${equipPrefix}] 🟢 REMOBILIZADO - Trabalhou/mobilizou após demob:`, entriesAfterDemob.map(e => e.date).join(', '));
            } else {
                console.log(`   [${equipPrefix}] 🔴 DESMOBILIZADO - Sem trabalho/mobilização após ${lastDemobDate}`);
...
const handleWorkSelectChange = async () => {
    ...
    if (entryEquipmentSelect) {
        ...
        if (selectedBm) {
            // Buscar status de mobilização de cada equipamento
            equipmentsWithStatus = await Promise.all(
                validEquipmentsInWork.map(async (equip) => {
            ...
            equipmentsWithStatus = validEquipmentsInWork.map(equip => ({
                ...equip,
                mobilizationStatus: { status: 'never_mobilized', lastMobilizationDate: null }
            }));
        }

        // Ordenar PRIMEIRO pela ordem no cadastro da obra, DEPOIS por status de mobilização
        const sortedEquipment = [...equipmentsWithStatus].sort((a, b) => {
            ...
            // PRIORIDADE 2: Status de mobilização (apenas se estiverem na mesma posição, o que não acontece)
            const priorityMap = {
                'mobilized': 1,
                'never_mobilized': 2,
                'demobilized': 3
        ...
        console.log('🎨 STATUS DE MOBILIZAÇÃO DOS EQUIPAMENTOS:');
        sortedEquipment.forEach(e => {
            console.log(`   ${e.prefix}: ${e.mobilizationStatus.status}`);
        });
        ...
        if (e.mobilizationStatus.status === 'demobilized') {
            ...
            console.log(`      ✅ [${e.prefix}] Aplicado: ⚪ CINZA (nunca mobilizado)`);
        } else {
    ...
    if (workId && bmIndex !== '') {
        ...
        if (entryEquipmentSelect) {
            ...
            const sortedEquipment = [...equipmentsWithStatus].sort((a, b) => {
                ...
                // PRIORIDADE 2: Status de mobilização
                const priorityMap = {
                    'mobilized': 1,
                    'never_mobilized': 2,
                    'demobilized': 3
...
const createEntryCard = (entryData, currentSubstitution = null) => {
    ...
    if (bmLabel && work?.config?.measurement_periods) {
        ...
        if (period && equipConfig) {
            ...
            const validDamages = appState.damages ? appState.damages.filter(damage =>
                ...
                damage.date >= period.start &&
                damage.date <= period.end
            ) : [];
    ...
    const stoppagesHtml = (entryData.stoppages || []).map(stoppage => createStoppageRow(stoppage)).join('');
    ...
    if (bmTotalValue !== null) {
        valuesLine += '<span style="color: #999;"> - </span><strong style="color: #FFD700; white-space: nowrap;">Total ' + (bmLabel || 'BM') + ': ' + formatCurrency(bmTotalValue) + '</strong>';
    }
    ...
    // Verificar paradas
    const stoppagesCount = (entryData.stoppages || []).filter(s => s.stoppage_type_id).length;
    if (stoppagesCount > 0) {
        alertBadges.push(`<span class="alert-badge alert-badge-stoppage">⚠️ PARADA REGISTRADA${stoppagesCount > 1 ? ` (${stoppagesCount})` : ''}</span>`);
    }
        ...
        <div class="entry-card collapsed" data-date="${entryData.date}" ${entryData.id ? `data-id="${entryData.id}"` : ''} data-work-id="${entryData.work_id}" data-equipment-id="${entryData.equipment_id}" data-bm-label="${bmLabel}">
            <div class="entry-card-header collapsible-header">
                ...
                <div class="summary-totals">
                    <strong>Horas Acumuladas: </strong><span class="cumulative-run-hours-display">0.00</span>
                    <strong style="margin-left: 15px;">KM Acumulados: </strong><span class="cumulative-run-km-display">0.00</span>
                </div>
...
const attachCardEventListeners = () => {
   entriesListContainer.querySelectorAll('.entry-card').forEach(card => {
       ...
       card.querySelector('.save-entry-btn-header')?.addEventListener('click', async () => {
           console.log('🖱️ BOTÃO SALVAR CLICADO! - VERSÃO NOVA 29/01 17:58');
           // ✅ saveEntryCard JÁ FAZ O RECÁLCULO AUTOMÁTICO após salvar e recarregar dados
           await saveEntryCard(card);
           console.log('✅ saveEntryCard CONCLUÍDA! - VERSÃO NOVA 29/01 17:58');
       });
...
const handleReplicateEntries = async () => {
    ...
    try {
        ...
        // 2. Para cada equipamento de destino
        for (const targetEquipId of targetEquipmentIds) {
            ...
            for (let d = new Date(startDateObj); d <= endDateObj; d.setDate(d.getDate() + 1)) {
                ...
                if (sourceEntry) {
                    ...
                    const basePayload = {
                        ...
                        operator_id: sourceEntry.operator_id || null,
                        horometer_start: sourceEntry.horometer_start || null,
                        horometer_end: sourceEntry.horometer_end || null,
                        km_start: sourceEntry.km_start || null,
                        km_end: sourceEntry.km_end || null,
                        is_mobilization: sourceEntry.is_mobilization || false,
                        is_mobilization_contabilized: sourceEntry.is_mobilization_contabilized || 1,
                        mobilization_manual_value: sourceEntry.mobilization_manual_value || null,
                        ...
                        demobilization_manual_value: sourceEntry.demobilization_manual_value || null,
                        stoppages: sourceEntry.stoppages || [],
                        notes: sourceEntry.notes || [],
                        pay_salary: sourceEntry.pay_salary !== false,
                        is_daily_part_received: sourceEntry.is_daily_part_received || false,
                        daily_total_value: 0,
        ...
        // 3. Recarregar a visualização com os dados atualizados
        displayPeriodEntries(workId, sourceEquipmentId, startDate, endDate);
        ...
        try {
            ...
            targetCards.forEach(card => {
                ...
                if (headerValuesContainer) {
                    ...
                    // Atualizar TOTAL da BM no bloco de info
                    const periodBmTotal = document.getElementById('period-bm-total');
                    if (periodBmTotal) {
                        periodBmTotal.textContent = `TOTAL DA ${bmLabel}: ${formatCurrency(bmTotalValue)}`;
                        console.log(`🔥 TOTAL DA BM ATUALIZADO: ${formatCurrency(bmTotalValue)}`);
                    }
