 ...
 */
export async function exportSandInvoicesReportToPDF(containerId, reportTitle, withCover) {
    showSpinner();
    ...
    try {
        ...
        printableElement.id = 'printable-clone-sand-invoices-report-pdf'; // ID único para o clone
        ...
        const addPageHeaders = (doc, data, sectionTitle) => {
            ...
            doc.text(reportInfo.reportSpecificTitle, margin, y); // Ex: Relatório de Notas Fiscais de Areia
            doc.text(`Status: ${reportInfo.period.replace('Período:', '').trim()}`, pdfWidth - margin, y, { align: 'right' }); // Reutiliza o campo período para o status
            y += lh;
            doc.text(`Período: ${reportInfo.period.replace('Status:', '').trim()}`, margin, y); // Reutiliza o campo período para o período de datas
        ...
        // Tabela de Resumo
        const summaryTable = printableElement.querySelector('#sand-invoices-summary-table');
        if (summaryTable) {
            ...
            pdf.autoTable({
                ...
                didDrawPage: (data) => addPageHeaders(pdf, data, 'Resumo por Empresa Cliente'),
            });
        }

        const fileName = generatePDFFileName(
            'Notas Fiscais Areia', // Nome base para o arquivo
            null, // Sem BM number
            reportInfo.startDate,
            reportInfo.endDate,
            reportInfo.reportSpecificTitle.replace('Relatório de Notas Fiscais de Areia', '').trim() // Usa o status como parte do nome
        ) + '.pdf';
        ...
        console.error("Erro ao gerar PDF do relatório de notas fiscais de areia:", e);
        alert("Não foi possível gerar o PDF do relatório de notas fiscais de areia. Detalhes: " + e.message);
        return null;
    } finally {
        hideSpinner();
