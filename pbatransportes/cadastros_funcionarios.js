...
const crudConfig = {
    ...
    title: 'Funcionários',
    fields: [
        { name: 'name', label: 'Nome do Funcionário', type: 'text', required: true }, 
        { name: 'role', label: 'Função', type: 'text' },
        { name: 'is_terceirizado', label: 'É Terceirizado?', type: 'checkbox' },
        { name: 'terceirizado_id', label: 'Empresa Terceirizada', type: 'select', optionsKey: 'terceirizados', dependsOn: 'is_terceirizado' }
    ],
    table: 'employees'
};

/**
 * Inicializa a interface de CRUD para Funcionários.
 * @param {string} key - A chave de configuração do CRUD (neste caso, 'employees').
 */
export const initCrudFuncionarios = async (key) => {
...
const loadAndRenderCrudData = async (key) => {
    ...
    // Carrega terceirizados se necessário
    ...
    try {
        ...
        data.forEach(item => {
            ...
            config.fields.forEach((f, i) => {
                ...
                if (f.name === 'terceirizado_id' && item[f.name]) {
