// relatorios_partes_diarias_not_received_pdf.js
import { showSpinner, hideSpinner } from './utils.js';

/**
 * Exporta o relatório de partes diárias não recebidas para PDF.
 ...
 */
export async function exportDailyPartsNotReceivedToPDF(containerId, reportTitle) {
    showSpinner();
    ...
    try {
        ...
        const notReceivedTableContainer = printableElement.querySelector('#daily-parts-not-received-table-container');
        if (notReceivedTableContainer) {
            const table = notReceivedTableContainer.querySelector('table');
            const tableTitle = notReceivedTableContainer.querySelector('h3')?.innerText || 'Partes Diárias Não Recebidas';
